...
// @param particleNum : 発生させる数
void AddParticle(UINT particleNum, ParticleManager* particleMgr, const XMFLOAT3& centerPos = XMFLOAT3()) {
	for (UINT i = 0; i < particleNum; i++) {
		// X,Y,Z全て[-5.0f,+5.0f]でランダムに分布
		constexpr float rnd_pos = 10.0f;
		XMFLOAT3 pos = centerPos;
		pos.x += (float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
		pos.y += (float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
		pos.z += (float)rand() / RAND_MAX * rnd_pos - rnd_pos / 2.0f;
		// X,Y,Z全て[-0.05f]でランダムに分布
		constexpr float rnd_vel = 0.1f;
		XMFLOAT3 vel{};
		...
		// 追加
		particleMgr->Add(60, pos, vel, acc, 1.0f, 0.0f);
	}
}
...
#pragma region 描画初期化処理
	...
	for (int i = 0; i < enemyNam; i++)
	{
		...
		angle[i] = (float)ran;
		//angle[i] = 60.0f;
		constexpr float rnd_acc = 0.0000f;
	...
	/*objPost->Update();
	objChr->Update();*/
	constexpr float backScale = 50.f;
	OBJBack->SetScale({ backScale,backScale,backScale });

	// --------------------
	// パーティクル
	// --------------------
	constexpr size_t particleNum = 20;
	std::unique_ptr<ParticleManager> particleMgr(ParticleManager::Create());
...
#pragma region DirectX毎フレーム処理
		...
		if (scene == 1)
		{
			...
			constexpr int cycle = 540; // 繰り返しの周期
			counter++;
			...
			if (input->PushKey(DIK_D))
			{
				...
				XMVECTOR playerReMoveVector3 = XMLoadFloat3(&playerReMove);
				spritePlayerRe->MoveVector(playerReMoveVector3);
...
#pragma region enemy hit 
			...
			if (input->TriggerMouseLeft() && thunderFlag == 0)
			{
				...
				if (isTerritory)
				{
					...
					AddParticle(particleNum, particleMgr.get(), particlePos);
				}
			}
			if (thunderFlag == 1)
			{

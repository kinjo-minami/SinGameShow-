...
bool ParticleManager::LoadTexture()
{
	...
	//result = LoadFromWICFile(
	//	L"Resources/kusa.png", WIC_FLAGS_NONE,
	//	&metadata, scratchImg);
	//if (FAILED(result)) {
	//	return result;
	//}
...
void ParticleManager::UpdateViewMatrix()
{
	...
	//視点座標
	XMVECTOR eyePoaition = XMLoadFloat3(&eye);
	//注意点座標
	XMVECTOR targetPoaition = XMLoadFloat3(&target);
	//（仮の）上方向
	XMVECTOR upVector = XMLoadFloat3(&up);

	//カメラZ軸（視点方向）
	XMVECTOR cameraAxisZ = XMVectorSubtract(targetPoaition, eyePoaition);
	...
	//視点座標に-1を掛けた座標
	XMVECTOR reverseEyePosition = XMVectorNegate(eyePoaition);
	//カメラの位置からワールド原点へのベクトル（カメラ系座標）
	XMVECTOR tX = XMVector3Dot(cameraAxisX, reverseEyePosition);//X成分
	XMVECTOR tY = XMVector3Dot(cameraAxisY, reverseEyePosition);//Y成分
	XMVECTOR tZ = XMVector3Dot(cameraAxisZ, reverseEyePosition);//Z成分
	//一つのベクトルにまとめる
	XMVECTOR translattion = XMVectorSet(tX.m128_f32[0], tY.m128_f32[1], tZ.m128_f32[2], 1.0f);

	//ビュー行列に平行移動成分を設定
	matView.r[3] = translattion;
...
void ParticleManager::DrawWithUpdate() {
	Update();
	Draw();
}
